...
fn conetrace(ray_pos: vec3f, ray_dir: vec3f, tan_angle: f32, max_dist: f32) -> vec4f {
    ...
    for (var i = 0u; i < #SHADOW_MAX_ITER && dist <= max_dist; i++) {
        ...
        let sample = textureSampleLevel(colors, nearest_sampler, pos / size, log2(radius));
...
fn trace_shadow(hit_pos: vec3f, ray_dir: vec3f) -> f32 {
    ...
    let max_dist = 1000.0;
    let pos = hit_pos + ray_dir * 0.01;
    let sample = conetrace(pos, ray_dir, shadow_spread, max_dist);
    return sample.a;
}
